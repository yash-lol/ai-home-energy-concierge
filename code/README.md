...
Three tiers, deliberately placed. The argument is not that the model is big; it
is that each job runs where its latency budget and the available silicon say it
should.

| Tier | Hardware | Model | Job | Measured |
|---|---|---|---|---|
| **1. Edge** | Dragonwing QRB2210, quad Cortex-A53, **CPU only** | logistic regression, 9 features | score every sample for anomaly | **30.6 us** |
| **2. Hub** | Snapdragon X Elite, 45 TOPS Hexagon NPU | Qwen3-4B-Instruct W4A16 via GenieX | plan synthesis across all findings | **11.5 s**, once per change |
| **3. Hub, on demand** | same NPU, same model | same | natural-language Q&A over live state | **~3.4 s**, streamed |

**Why not an LLM on the UNO Q?** That board has no on-device AI acceleration
stack at all - no DSP skel, no SNPE, no onnxruntime; its GPU is Mesa `rusticl`
software rasterisation. A 4B model there is physically wrong. But *AI at the
edge does not have to mean an LLM*: a trained classifier runs there in
microseconds, in pure Python, with zero new dependencies. That split is the
design, not a compromise.

**What each tier decides, and what it is not allowed to decide:**

- **Tier 1** decides *whether the current state is unusual for this home*. It
  catches what no fixed threshold expresses - e.g. `3 AM, room occupied, 24 C,
  A/C running, off-peak`, where all 7 rules produce nothing and the classifier
  scores **0.805**. It never touches a dollar figure.
- **Tier 2** decides *the order to act in*, which findings interact, and what can
  wait. Given the same three findings, the deterministic path ranks the dryer
  first because it costs most; the model defers it as legitimate use with a
  timing problem and promotes the A/C cooling an empty home. Numbers are still
  Python's.
- **Tier 3** answers questions. Same rule: it may cite only figures from a digest
  computed in Python.

**Nothing that runs per-sample calls the NPU.** The 3.1-11.5 s tier is invoked
once per *change*, never in a loop.

### The anomaly model is trained on simulated data - stated plainly

`hub/anomaly_model.py` is generated by `tools/train_anomaly.py` from **14
simulated days** of household rhythm (840 samples, 168 anomalous). Holdout
accuracy **0.9714**, precision 0.947, recall 0.900, seed fixed so the figures
reproduce.

Those numbers describe how separable a *synthetic* distribution is. They are
**not** evidence of real-world accuracy. In deployment the model would retrain on
real logged history. This caveat is written into the generated model file, into
`anomaly.model_provenance()`, and into every evidence line the detector emits -
so a learned score can never reach a user looking like a measurement.

The learned detector and the rules cover **different ground**, which is the
honest claim rather than "the model catches everything":

| Case | Rules fired | Learned score |
|---|---|---|
| 3 AM, A/C on, room occupied, off-peak | **0** | **0.805** (model catches it) |
| Lights on in bright daylight while away | 1 (R3) | 0.626 (model misses; R3 owns it) |
| All loads on, nobody home | 2 | 0.997 (both agree) |

### Numeric provenance: we check, we do not just ask

Every LLM response goes through `hub/provenance.py`, which extracts every numeric
literal and verifies it appears in the deterministic source the model was given.
Unverified answers are **labelled, not hidden**.

It caught this during development, unprompted:
...
`$0.39` was in the digest. `$0.19` was not - the model did arithmetic it had been
told not to do. Most systems answer *"how do you know it didn't make that up?"*
with "we told it not to." This answers with a mechanism and a violation count.
...
|---|---|---|---|---|
| **Edge anomaly inference (tier 1)** | **30.6** | **32.4** | **us** | **on the UNO Q's Dragonwing A53**, pure Python, no numpy |
| Energy-model estimate | 0.002 | 0.002 | ms | pure arithmetic + formula string |
| Rules engine (7 rules) | 0.014 | 0.016 | ms | 1 room / 3 loads -> 2 findings |
| Provenance verification (per response) | 110.8 | 138.7 | us | every number checked against its deterministic source |
| Narration - deterministic template | 0.001 | 0.002 | ms | control path; no model, no network |
| Plan synthesis - deterministic fallback | 0.002 | 0.005 | ms | tier 2 fallback, pure Python ranking |
| Sensor snapshot -> recommendations | 0.020 | 0.092 | ms | rules + energy model + narration |
| Fixed batch - 25 findings ranked | 0.005 | 0.011 | ms | seed 20260806, reproducible |
| Narration - local LLM (tier 2, GenieX NPU) | 3327.9 | 3481.1 | ms | `qualcomm/Qwen3-4B-Instruct-2507` (W4A16) |
| **Plan synthesis (tier 2, GenieX NPU)** | **11.5** | **11.7** | **s** | one call per **change** of the finding set, not per cycle |
| Q&A round-trip (tier 3, GenieX NPU) | ~3.4 | - | s | first token ~2.4 s, streamed |
...
> **The rules-engine row predates R8 and the learned tier.** It was measured with seven
> rules and no anomaly scorer; there are now eight rules plus tier 1. Re-run
> `python hub/benchmark.py --markdown` on the demo machine and paste the new table
> rather than relabelling this one — the figure above is what was actually measured,
> and it should stay that way until it is measured again.

**Latency is dominated by OUTPUT LENGTH, not model size.** Measured on this NPU:
135 chars -> 2.6 s, 378 -> 5.5 s, 1060 -> 11.4 s, ~600 tokens -> ~135 s. That is
why every prompt in this project demands brevity, and why `LLM_MAX_TOKENS`
defaults to 160. It is a performance decision, not a style one. (An earlier
build requested 300 tokens with no brevity instruction and took ~11.4 s against
its own 8 s timeout, so narration silently fell back to the template every
single time. Capping the output made it 5x faster *and* actually use the NPU.)
